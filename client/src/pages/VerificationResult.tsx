...
export default function VerificationResult() {
  ...
  // Generate QR code that links to verification lookup page
  const verificationLookupUrl = `${window.location.origin}/verify/lookup?code=${result.verificationCode}`;
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(verificationLookupUrl)}`;
...
Generated by TrustLink - Smart Digital Identity Verification System
  ...
  const handleCopyLink = async () => {
    try {
      await navigator.clipboard.writeText(verificationLookupUrl);
  ...
  const handleShareTwitter = () => {
    ...
    const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(verificationLookupUrl)}`;
  ...
  const handleShareLinkedIn = () => {
    const url = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(verificationLookupUrl)}`;
    window.open(url, "_blank", "width=550,height=420");
    setShowShareMenu(false);
  ...
  const handleShareFacebook = () => {
    const url = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(verificationLookupUrl)}`;
    window.open(url, "_blank", "width=550,height=420");
    setShowShareMenu(false);
